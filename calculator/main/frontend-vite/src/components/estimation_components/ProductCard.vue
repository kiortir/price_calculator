...
export default {
    ...
    computed: {
        product() {
            return this.$store.state.products.values[this._id];
        },
        productProps() {
            let props = Object.assign({}, this.product)
            delete props.p_type
            return props || {}
        },
        fields() {
...
<template>
    <div
        ...
        :ref="`product_card_${_id}`"
    >
        <div :ref="`product_card_${_id}_child`" class="preview">
            <div class="title rounded-t-lg flex flex-row divide-x-2 divide-gray-500 h-full">
                ...
                >{{ fields.p_type.options[product.p_type] || 'Изделие' }}</div>
                <div
                ...
                <div
                    ...
                    @click="$emit('edit', 'product-card-edit')"
                >
